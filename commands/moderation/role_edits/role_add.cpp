...
static void handle_role_log_select(dpp::cluster& bot, const dpp::select_click_t& event) {
	...
	try {
		...
		event.co_edit_response(dpp::message(msg_content).set_flags(dpp::m_ephemeral));
	}
	catch (const std::exception& e) {
		get_logger()->log(LogLevel::Error, "Failed to save log channel to JSON: " + std::string(e.what()), false);
		event.co_edit_response(dpp::message("An error occurred while saving your selection. Please inform <@" + secrets.at("OWNER_ID") + "> of this.").set_flags(dpp::m_ephemeral));
	}
}
